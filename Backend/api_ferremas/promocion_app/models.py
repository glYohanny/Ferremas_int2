...
from producto_app.models import Producto, Categoria, Marca # Asegúrate de que estos modelos existan
...
class Promocion(models.Model):
    ...
    # limite_uso_por_cliente = models.PositiveIntegerField(blank=True, null=True, verbose_name="Límite de Uso por Cliente") # Considerar si se necesita
    ...
    def esta_vigente(self):
        ...
        now = timezone.now()
        # Asegurarse que la fecha de inicio se considera desde el comienzo del día
        # y la fecha de fin hasta el final del día.
        # Si fecha_fin es YYYY-MM-DD (implica 00:00:00), la promoción es válida
        # hasta justo antes del inicio del día siguiente.
        fecha_fin_limite = self.fecha_fin + timedelta(days=1)

        return self.activo and self.fecha_inicio <= now < fecha_fin_limite
    @property
    def esta_vigente_display(self): # Usado por el serializer para el frontend
        return self.esta_vigente()

    @property
    def tipo_promocion_display(self): # Usado por el serializer para el frontend
        return self.get_tipo_promocion_display()

    @property
    def objetivo_promocion_detalle(self): # Usado por el serializer para el frontend
        if self.objetivo_promocion:
            return {"id": self.objetivo_promocion.id, "nombre": str(self.objetivo_promocion)}
        return None

    def aplicar_a_precio(self, precio_original: float) -> float:
        if not self.esta_vigente or self.valor is None: # Quitar paréntesis
            return precio_original
        if self.tipo_promocion == self.TipoPromocion.DESCUENTO_PORCENTAJE: # Usar el nombre correcto del miembro
            return max(0, precio_original - (precio_original * (self.valor / 100)))
        elif self.tipo_promocion == self.TipoPromocion.PRECIO_FIJO:
            return Decimal(self.valor) # Devolver como Decimal
        elif self.tipo_promocion == self.TipoPromocion.DESCUENTO_MONTO_FIJO:
            return max(Decimal('0'), precio_original - Decimal(self.valor)) # Operar con Decimal
        # Para 2X1 o REGALO, el precio unitario no cambia aquí; se maneja en la lógica del carrito.
        return precio_original
