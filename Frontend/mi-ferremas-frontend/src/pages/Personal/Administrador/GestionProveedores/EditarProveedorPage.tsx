// src/pages/Personal/Administrador/GestionProveedores/ProveedorFormPage.tsx
...
interface Region { // Interfaz para las regiones
    id: number;
    nombre: string;
}
...
const ProveedorFormPage: React.FC = () => {
    ...
    const [regiones, setRegiones] = useState<Region[]>([]); // Estado para las regiones
    const [selectedRegion, setSelectedRegion] = useState<number | ''>(''); // Estado para la región seleccionada
            ...
            try {
                ...
                if (isEditMode) {
                    ...
                    setFormData({
                        ...
                        comuna: providerData.comuna || '', // ID de la comuna
                    ...
                    // Si hay una comuna, intentar preseleccionar la región
                    if (providerData.comuna) {
                        // Para obtener la región de la comuna, necesitamos todas las comunas
                        // o que el endpoint del proveedor devuelva la región de la comuna.
                        // Asumimos que el serializer de Proveedor ya incluye comuna_detalle con region_detalle
                        if (providerData.comuna_detalle?.region_detalle?.id) {
                            setSelectedRegion(providerData.comuna_detalle.region_detalle.id);
                        } else {
                            // Fallback si no viene anidado: cargar todas las comunas para encontrar la región
    ...
    useEffect(() => {
        if (selectedRegion) {
            const fetchComunasPorRegion = async () => {
                try {
                    ...
                    setComunas([]); // Limpiar comunas en caso de error
                }
            };
    ...
    const handleRegionChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
        ...
        setFormData(prev => ({ ...prev, comuna: '' })); // Resetear comuna al cambiar de región
    };
            ...
            <form onSubmit={handleSubmit} className="space-y-8">
                ...
                <section className="p-6 border border-gray-200 rounded-lg">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        {/* Selector de Región */}
                        <select
                            name="region" // El nombre no se usa directamente para el form data, pero es bueno tenerlo
                        ...
                        {/* Selector de Comuna (dependiente de la región) */}
